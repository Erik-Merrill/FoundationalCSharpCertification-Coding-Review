...
while (transactions > 0)
{
    ...
    debugCheck = cashTill[3] * 20 + cashTill[2] * 10 + cashTill[1] * 5 + cashTill[0];
    Console.WriteLine(TillAmountSummary(cashTill));
    Console.WriteLine($"Expected till value: {registerCheckTillTotal}");
    Console.WriteLine();
}
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    int twentiesOnHand = cashTill[3] + twenties;
    int tensOnHand = cashTill[2] + tens;
    int fivesOnHand = cashTill[1] + fives;
    int onesOnHand = cashTill[0] + ones;
    ...
    /*
        ...
        cashTill[0] += ones;
    */
    int[] preparedChange = new int[] { 0, 0, 0, 0 };

    Console.WriteLine("Cashier prepares the following change:");

    while ((changeNeeded > 19) && (twentiesOnHand > 0))
    {
        twentiesOnHand--;
        //cashTill[3]--;
        changeNeeded -= 20;
    ...
    while ((changeNeeded > 9) && (tensOnHand > 0))
    {
        tensOnHand--;
        //cashTill[2]--;
        changeNeeded -= 10;
    ...
    while ((changeNeeded > 4) && (fivesOnHand > 0))
    {
        fivesOnHand--;
        changeNeeded -= 5;
    ...
    while ((changeNeeded > 0) && (onesOnHand > 0))
    {
        onesOnHand--;
        //cashTill[0]--;
        changeNeeded -= 1;
    ...
    cashTill[3] = twentiesOnHand;
    cashTill[2] = tensOnHand;
    cashTill[1] = fivesOnHand;
    cashTill[0] = onesOnHand;
